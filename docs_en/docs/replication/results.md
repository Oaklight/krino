...
This document consolidates findings from our effort to build an open typed decision model reproducing the pipeline identified through [black-box probing](../probing/architecture.md) of TypeSafe's Jev:
...
5. **CodeSearchNet 100% for rerankers is a length-bias artifact.** The correct code snippet is 2× longer on average than distractors. Rerankers exploit length/lexical overlap rather than semantic understanding.
...
**Jev latency:** Consistent ~305ms TTFB across all benchmarks (measured from our client). Higher than the probing-era measurements (median 166ms) — likely due to benchmark payloads being larger than typical probing payloads, or API infrastructure changes.

### Key observations

1. **Jev dominates on reasoning-heavy tasks.** ARC (99.0%), authored144 (97.9%), RACE (95.5%), HellaSwag (89.5%) — these all require multi-hop reasoning, reading comprehension, or commonsense inference. Our best zero-shot models are 20-43pp behind. This strongly suggests Jev uses a large causal backbone with substantial reasoning capability — an encoder-only model cannot achieve 99% on ARC.
...
4. **Score-type tasks are where Jev's advantage is clearest.** Our zero-shot models get MAE 0.85+ on SST-5/STS-B/Yelp; Jev gets 0.43-0.49. The logit-readout and cross-encoder approaches don't handle ordinal regression well.

5. **The causal backbone question is settled.** A pure encoder (Ettin-150m) cannot reach Jev-level performance on ARC, RACE, or authored144 — these require sequential reasoning that causal LMs excel at.

### Implications for training strategy

The Jev API results reshape our priorities:

- **Multi-task training on all 19 benchmarks** is the immediate next step
- **Causal backbone (Qwen) may be needed** for reasoning-heavy tasks, despite encoder's efficiency advantage on comparison tasks
- **Calibration is Jev's core advantage.** Step 3 calibration training is the path to closing this gap
- **Score-type performance needs a different approach.** MAE 0.85 vs Jev's 0.43 — ordinal losses or RLCD with score-specific rewards are needed
...
1. **Reranker pretraining transfers to decision scoring.** Ettin-150m (93.8%) beats vanilla ModernBERT-base (89.0%) at identical param count (150M). Same architecture, different pretraining — the only variable is whether the backbone was pretrained on 143M relevance-scoring triples. This is a +4.8pp improvement.
...
3. **Training > scaling.** A 0.6B causal LM with 402K trained heads (93.2%) beats a 7B causal LM with zero-training logit readout (68.5% average across 19 benchmarks).

4. **Ettin-400m underperforms Ettin-150m** (91.6% vs 93.8%). Training instability at epoch 4 (accuracy dropped 89.0% → 75.2% before recovering) suggests the 1e-3 learning rate is too aggressive for the larger backbone.
...
R² of surface-form features vs assigned probability, measured on semantically equivalent options with varied wording:
...
- **Logit readout** is dominated by grammaticality (R²=0.96) — the causal LM assigns near-zero probability to ungrammatical continuations.
- **Trained causal heads** shift to length and commonness (R²=0.72, 0.65) — the heads partially learn to ignore grammaticality but pick up different biases.
...
The key difference from standard LTR: decision models need **calibrated probabilities**, not just correct ranking. A reranker only needs to order documents correctly; a decision model's probabilities must be trustworthy.
...
- **Step 4:** Compact custom architecture exploration (partially addressed by Ettin findings)
...
3. **Encoder vs causal backbone for production?** Ettin-150m wins on comparison tasks (Banking77 95.2%), but Jev API results show reasoning-heavy tasks require causal LM representations.

4. **Can we close the score-type gap?** Jev scores MAE 0.43–0.49 on SST-5/STS-B/Yelp; our models get 0.85+. Ranked probability score or ordinal-specific losses may be needed.

5. **What is Jev's calibration advantage?** Jev achieves ECE 0.009 on ARC and 0.031 on authored144. Our models' ECE is unmeasured on trained heads — this is the key metric for Step 3.
